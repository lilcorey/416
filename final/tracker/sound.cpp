  ...
  channels()
{
  ...
  music = Mix_LoadMUS("sounds/69_Forest_Night.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load ForestMusik")+Mix_GetError();

  startMusic();

   sounds.push_back( Mix_LoadWAV("sounds/dogtoy.wav") );
